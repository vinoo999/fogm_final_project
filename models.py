...
from edward.models import Gamma, Poisson, PointMass, Uniform, Dirichlet, Empirical, Categorical, Mixture, Independent, Bernoulli, Multinomial
...
class PoissonMF(object):
    ...
    def train(self, n_iter=1000):
        with self.sess:
            ...
            # INFERENCE
            # Last ditched
            # self.qpi = Empirical(tf.get_variable(
            #         "qpi/params", [steps,self.num_clusters],
            #         initializer=tf.constant_initializer(1.0 / self.num_clusters)))
            # self.qz = PointMass(tf.get_variable(
            #     "qz/params", [self.n_cells],
            #     initializer=tf.zeros_initializer(),
            #     dtype=tf.int32))
            # self.qz = Empirical(tf.get_variable(
            #     "qz/params", [steps, self.n_cells],
            #     initializer=tf.zeros_initializer(),
            #     dtype=tf.int32))
            # self.qrU = Empirical(tf.nn.softplus(tf.Variable(tf.random_normal([steps,self.num_clusters, self.latent_dim]))),)
            # self.qrV = Empirical(tf.nn.softplus(tf.Variable(tf.random_normal([steps,self.n_genes]))),)
            ...
            # inference_e1 = ed.Gibbs(
            #     {self.pi:self.qpi}, 
            #     data={self.z: self.z})
            
            # inference_e2 = ed.MAP(
            #     {self.z: self.qz, self.rU:self.qrU, self.rV:self.qrV}, 
            #     data={self.U:self.qU, self.V:self.qV})
            ...
            # # optimizer2 = tf.train.AdamOptimizer(learning_rate=0.001, beta1=0.9, beta2=0.999,epsilon=1e-08)
            # optimizer3 = tf.train.AdamOptimizer(learning_rate=0.001, beta1=0.9, beta2=0.999,epsilon=1e-08)
            # inference_e.initialize(n_iter=n_iter, optimizer=optimizer2)
            # inference_e1.initialize()
            # inference_e2.initialize(n_iter=n_iter, optimizer=optimizer3)
            # inference_m.initialize(n_iter=n_iter, optimizer=optimizer)
            ...
            inference.initialize(n_iter=n_iter, optimizer=tf.train.AdamOptimizer
                             (learning_rate=0.001, beta1=0.9, beta2=0.999,
                              epsilon=1e-08))
            # inference.initialize(n_iter=n_iter, optimizer='rmsprop')
            tf.global_variables_initializer().run()
            self.loss = np.empty(n_iter, dtype=np.float32)
            for i in range(n_iter):
                # info_dict_e = inference_e.update()
                # inference_e1.run()
                # info_dict_e2 = inference_e2.update()
                # info_dict_m = inference_m.update()
                # self.loss[i] = info_dict_m["loss"]
...
class PoissonHierarchical(object):
    ...
    def train(self, n_iter = 500, t = 500):
        with self.sess:
            rU = Gamma(1.0, 1.0, sample_shape=self.n_cells) 
            U = Gamma(1.0, rU, sample_shape=self.latent_dim) 

            rV = Gamma(0.1, 0.1, sample_shape=self.n_genes) 
            V = Gamma(1.0, rV, sample_shape=self.latent_dim) 
            ...
            inference_m = ed.MAP(
                {U: qU, V: qV},
                data = {R: self.data, rU: qrU, rV: qrV},
            )

            inference_e.initialize()
            # inference_m.initialize(n_iter=n_iter, optimizer=tf.train.AdamOptimizer
...
class PoissonHiS(object):
    ...
    def train(self, n_iter = 500, t = 500):
        with self.sess:
            ...
            p = 0.3 * tf.ones(self.n_genes)
            D = Multinomial(probs=p, total_count=float(self.n_cells))
            X = tf.multiply(R,D)
            ...
            inference_m = ed.MAP(
                {U: qU, V: qV},
                data = {X: self.data, rU: qrU, rV: qrV},
            )
            ...
            #                  (learning_rate=0.01, beta1=0.9, beta2=0.999,
            #                   epsilon=1e-08))
            inference_m.initialize(n_iter=n_iter, optimizer=tf.train.RMSPropOptimizer
                             (learning_rate=0.01))
